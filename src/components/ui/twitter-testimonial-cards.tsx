...
function TestimonialCard({
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex h-auto min-h-[140px] sm:min-h-[180px] w-[260px] sm:w-[380px] -skew-y-[8deg] select-none flex-col rounded-2xl border border-primary/20 bg-primary/5 backdrop-blur-sm px-3 sm:px-4 py-3 sm:py-4 transition-all duration-500 hover:border-primary/40 hover:bg-primary/10 cursor-pointer",
    ...
    >
      ...
      <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
        <div className="size-9 sm:size-12 rounded-full bg-gradient-to-br from-primary via-primary/70 to-primary/50 flex items-center justify-center overflow-hidden shrink-0">
          {avatar ? (
...
export default function Testimonials({ cards }: TestimonialsProps) {
  ...
  return (
    <div className="grid [grid-template-areas:'stack'] place-items-center opacity-100 animate-in fade-in-0 duration-700">
      {defaultCards.slice(0, 3).map((cardProps, index) => (
        <TestimonialCard
          key={index}
          {...cardProps}
